   ...
   */
   const performSearch = async (text) => {
    try {
      ...
      return setAllProducts(searchData.data);
    } catch (e) {
      console.log(e);
      setAllProducts([]);
    }
...
<Grid container>
      {loading ? (
        <div className="loading">
          ...
          <Typography variant="subtitle1">Loading Products</Typography>
        </div>
      ) : allProducts.length > 0 ? (
        
          <Grid item className="product-grid" md={token?9:12}>
            <Grid item className="product-grid" container>
              <Box className="hero">
            ...
            <Grid container marginY="1rem" paddingX="1rem" spacing={2}>
              ...
              {allProducts.map((listProduct) => (
                <ProductCard  product={listProduct}
