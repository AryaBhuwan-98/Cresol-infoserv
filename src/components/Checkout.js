...
const Checkout = () => {
  ...
  const [addresses, setAddresses] = useState({ all: [], selected: null });
  ...
  const validateRequest = (items, addresses) => {
    let balance = getTotalCartValue(items);
    let CheckoutBalance =  localStorage.getItem("balance")  
  ...
  } catch(e)
  {
    if(e.response)
     ...
     enqueueSnackbar("Could not preform Checkout. Check that the backend is running, reachable and returns valid JSON.",{variant:"error"}) ;
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
           ...
           {!newAddress.isAddingNewAddress && addresses._id!==null?(<Button
                color="primary"
                variant="contained"
                id="add-new-btn"
                size="large"
